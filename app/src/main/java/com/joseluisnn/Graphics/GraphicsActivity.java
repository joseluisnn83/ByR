package com.joseluisnn.Graphics;
...
import com.joseluisnn.tt.PreferencesGraphicsActivity;
...
public class GraphicsActivity extends Activity {
	...
	private void cargarValoresHistoricos() {
		...
		case 0: // ANUAL
			...
			// Obtengo los acumulados históricos hasta el inicio de la gráfica a
			// mostrar
			acumIngresos = dba.obtenerAcumulados(enteroFechaInicial, "ingreso");
			acumGastos = dba.obtenerAcumulados(enteroFechaInicial, "gasto");
			acumBalance = acumIngresos - acumGastos;
				...
				for (int i = integerFechaInicial.intValue(); i <= integerFechaFinal
					...
					if (indiceAux < listadoValoresIngresos.size()) {
						...
						if (Integer.valueOf(
							...
							ingresos.add(redondearValor(listadoValoresIngresos
									.get(indiceAux).getCantidad()));

							if (listadoValoresIngresos.get(indiceAux)
									.getCantidad() >= max) {
								max = redondearValor(listadoValoresIngresos
										.get(indiceAux).getCantidad());
							}

							if (listadoValoresIngresos.get(indiceAux)
									.getCantidad() <= min) {
								min = redondearValor(listadoValoresIngresos
										.get(indiceAux).getCantidad());
							}
				...
				for (int i = integerFechaInicial.intValue(); i <= integerFechaFinal
					...
					if (indiceAux < listadoValoresGastos.size()) {
						...
						if (Integer.valueOf(
							...
							gastos.add(redondearValor(listadoValoresGastos.get(
									indiceAux).getCantidad()));

							if (listadoValoresGastos.get(indiceAux)
									.getCantidad() >= max) {
								max = redondearValor(listadoValoresGastos.get(
										indiceAux).getCantidad());
							}

							if (listadoValoresGastos.get(indiceAux)
									.getCantidad() <= min) {
								min = redondearValor(listadoValoresGastos.get(
										indiceAux).getCantidad());
							}
			...
			for (int i = 0; i < ingresos.size(); i++) {
				// balance.add(redondearValor(ingresos.get(i) - gastos.get(i)));

				acumBalance = redondearValor(acumBalance
						+ (ingresos.get(i) - gastos.get(i)));
				balance.add(acumBalance);
			}
		...
		case 2: // DIARIO
			...
			/*
			 *  Obtengo los acumulados históricos hasta el inicio de la gráfica a mostrar
			 */
			acumIngresos = dba.obtenerAcumulados(enteroFechaInicial, "ingreso");
			acumGastos = dba.obtenerAcumulados(enteroFechaInicial, "gasto");
			acumBalance = acumIngresos - acumGastos;
	...
	private void cargarValoresAnualesPrevision() {
		...
		// Obtengo los acumulados históricos hasta el inicio de la gráfica a
		// mostrar
		acumIngresosPrevision = dba.obtenerAcumulados(enteroFechaInicial,
				"ingreso");
		acumGastosPrevision = dba
				.obtenerAcumulados(enteroFechaInicial, "gasto");
			...
			for (int i = integerFechaInicial.intValue(); i <= integerFechaUmbralHistorico
				...
				if (indiceAux < listadoValoresIngresos.size()) {
					...
					if (Integer.valueOf(
						...
						ingresos.add(redondearValor(listadoValoresIngresos.get(
								indiceAux).getCantidad()));

						if (listadoValoresIngresos.get(indiceAux).getCantidad() >= max) {
							max = redondearValor(listadoValoresIngresos.get(
									indiceAux).getCantidad());
						}

						if (listadoValoresIngresos.get(indiceAux).getCantidad() <= min) {
							min = redondearValor(listadoValoresIngresos.get(
									indiceAux).getCantidad());
						}
			...
			for (int i = integerFechaUmbralPrevision.intValue(); i <= integerFechaFinal
				...
				if (indiceAux < listadoValoresIngresosPrevision.size()) {
					if (Integer.valueOf(
						...
						ingresos.add(redondearValor(listadoValoresIngresosPrevision
								.get(indiceAux).getCantidad()));

						if (listadoValoresIngresosPrevision.get(indiceAux)
								.getCantidad() >= max) {
							max = redondearValor(listadoValoresIngresosPrevision
									.get(indiceAux).getCantidad());
						}

						if (listadoValoresIngresosPrevision.get(indiceAux)
								.getCantidad() <= min) {
							min = redondearValor(listadoValoresIngresosPrevision
									.get(indiceAux).getCantidad());
						}
			...
			for (int i = integerFechaUmbralPrevision.intValue(); i <= integerFechaFinal
				...
				if (indiceAux < listadoValoresGastosPrevision.size()) {
					...
					if (Integer.valueOf(
						...
						gastos.add(redondearValor(listadoValoresGastosPrevision
								.get(indiceAux).getCantidad()));

						if (listadoValoresGastosPrevision.get(indiceAux)
								.getCantidad() >= max) {
							max = redondearValor(listadoValoresGastosPrevision
									.get(indiceAux).getCantidad());
						}

						if (listadoValoresGastosPrevision.get(indiceAux)
								.getCantidad() <= min) {
							min = redondearValor(listadoValoresGastosPrevision
									.get(indiceAux).getCantidad());
						}
		...
		for (int i = 0; i < NUMERO_ANYOS_HISTORICOS_PREVISON; i++) {
			// balance.add(redondearValor(ingresos.get(i) - gastos.get(i)));
			acumBalancePrevision = redondearValor(acumBalancePrevision
					+ (ingresos.get(i) - gastos.get(i)));
	...
	private void configurarGrafica() {
		...
		mySimpleXYPlot.setRangeBoundaries(minCantidad - 100, maxCantidad + 150,
				BoundaryMode.FIXED);
