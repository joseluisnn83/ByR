...
public class PestanyaSemanalActivity extends Activity{
	...
	/*
	 *  Variable para captar cuando es la primera ejecución del programa 
	 *  y controlar los cambios en el Spinner de las semanas
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapterSemanas = new ArrayAdapter<String>(
				this, 
		...
		rlBarraListadoIngresos.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Veo cual es el estado del listado de ingresos y luego lo abro o lo cierro
		...
		semanas.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1, int position, long arg3) {
				...
				if(posicionSpinnerAnterior != position){
					...
					if(position == 0){
						// entra por aquí si se quiere ver el listado de la semana anterior
						...
						// entra por aquí si se quiere ver el listado de la semana actual
	...
	/*
	 * Método que me carga en el Layout de listado de Ingresos las fechas, conceptos
	 * y valores asociados a partir de la consulta realizada a la Base de datos
	 * Mostrará valores de la semana actual o la semana anterior
	 */
	private void cargarLayoutListadoIngresos(){
		...
		for (int i=0;i<listadoValoresIngresos.size();i++){
			 ...
			 */
			rowViewContenido = (RelativeLayout)LayoutInflater.from(this).inflate(
					this.getResources().getLayout(R.layout.listado_informes_contenido),
					null);
			// Buscamos el View que contendrá el Concepto
			TextView tvConcepto = (TextView)rowViewContenido.findViewById(R.id.textViewListadoInformesConcepto);
			...
			TextView tvCantidad = (TextView)rowViewContenido.findViewById(R.id.textViewListadoInformesCantidad);
			// establecemos el contenido
			...
			if(fechaAnterior != listadoValoresIngresos.get(i).getIdFecha()){
				...
				rowViewFechas = (RelativeLayout)LayoutInflater.from(this).inflate(
						this.getResources().getLayout(R.layout.listado_informes_encabezado_fecha),
						null);
				
				TextView tvFecha = (TextView)rowViewFechas.findViewById(R.id.textViewEncabezadoFechaListadoInformes);
				tvFecha.setText(obtenerCadenaFecha(listadoValoresIngresos.get(i).getIdFecha()));
	...
	/*
	 * Método que me carga en el Layout de listado de Gastos las fechas, conceptos
	 * y valores asociados a partir de la consulta realizada a la Base de datos
	 * Mostrará valores de la semana actual o la semana anterior
	 */
	private void cargarLayoutListadoGastos(){
		...
		for (int i=0;i<listadoValoresGastos.size();i++){
			...
			if(fechaAnterior != listadoValoresGastos.get(i).getIdFecha()){
				...
				TextView tvFecha = (TextView)rowViewFechas.findViewById(R.id.textViewEncabezadoFechaListadoInformes);
				tvFecha.setText(obtenerCadenaFecha(listadoValoresGastos.get(i).getIdFecha()));
	...
	private int obtenerFinEnteroFechaSemanaActual(){
		...
		/*
		 *  Si el día solo tiene un dígito le pongo un cero delante
		 *  Le estoy sumando 7 para que me de el útimo día de la semana
		 */		
		day = "" + c.get(Calendar.DAY_OF_MONTH);
		if(day.length() == 1){
			day = "0" + day;
	...
	private int obtenerInicioEnteroFechaSemanaAnterior(){
		...
		// Obtengo de la variable Calendar la fecha de la semana anterior en el lunes
		Calendar c = Calendar.getInstance();
	...
	private int obtenerFinEnteroFechaSemanaAnterior(){
		...
		// Obtengo de la variable Calendar la fecha de la semana anterior en el domingo
		Calendar c = Calendar.getInstance();
		c.add(Calendar.WEEK_OF_YEAR, -1);
		c.set(Calendar.DAY_OF_WEEK, Calendar.SUNDAY);
	...
	/*
	 * Método que me calcula el total de los Ingresos a partir de la Lista de Ingresos 
	 * recuperada de la BD
	 */
	...
	/*
	 * Método que me calcula el total de los Gastos a partir de la Lista de Gastos 
	 * recuperada de la BD
	 */
	...
	private void actualizarTotales(){
		...
		tvTotalIngresos.setTextColor(this.getResources().getColor(R.color.ListadosVerdeOscuro));
		...
		if(balance >= 0){
			tvTotalBalance.setTextColor(this.getResources().getColor(R.color.ListadosVerdeOscuro));
		}else{
			tvTotalBalance.setTextColor(this.getResources().getColor(R.color.ListadosRojo));
		}
