...
public class DBAdapter {
    ...
    // otra prueba para evitar instanciación múltiple : es un Singleton para evitar instanción múltiple al objeto
	...
	public void borrarConcepto(Concepto c){
		...
		/*
		 * primero ver si hay dependencia con valores de la tabla datos; si los hubiera
		 * habría que borrar primero los datos de la tabla datos que tienen conceptos como
		 * clave ajena 
		 */
		try {
			
			database.execSQL("DELETE FROM conceptos WHERE id_concepto=" + c.getId() );
	...
	public ArrayList<Concepto> listarConceptos(String tipo){
		...
		String consulta = "select * from conceptos " +
				"where tipo_concepto='" +tipo+ "' " +
				"and  nombre not like 'ingreso_borrado' " +
		...
		while (cursor.moveToNext()){
			lista.add(new Concepto(
					cursor.getInt(cursor.getColumnIndex("id_concepto")), 
					cursor.getString(cursor.getColumnIndex("nombre")), 
					cursor.getString(cursor.getColumnIndex("tipo_concepto"))));
		}
	...
	public ArrayList<String> listarArrayConceptos(String tipo){
		...
		while (cursor.moveToNext()){
			lista.add(new String(cursor.getString(cursor.getColumnIndex("nombre"))));
		}
	...
	public int getIdConcepto(String nombre, String tipo){
		...
		String consulta = "select id_concepto from conceptos where tipo_concepto='" +tipo+ "' and nombre='" +nombre+ "'";
		int id_concepto=-1;
	...
	/*
	 * Método que me comprueba si hay mas valores de la tabla DATOS donde tengan la misma
	 * fecha pasado por parámetro.
	 * -Si existen datos con esa fecha no borro dicha fecha de la tabla Fechas
	 * -Si no hay datos con la fecha pasada por parámetro entonces la borro de la tabla Fechas
	 * ya que no me sirve 
	 */
	public int comprobacionDatosConEstaFecha(int fecha){
		...
		String consulta = "select count(*)  as 'numeroFechas' " +
						"from datos " +
	 ...
	 */
	public boolean borrarValor(int idConcepto, int fecha){
		
		int registro=1;
		
		String consulta = "delete from datos " +
	...
	/*
	 * Método que me borra todos los valores de la tabla DATOS según el idConcepto 
	...
	public int borrarValoresSegunConcepto(Concepto c){
		...
		String consulta1 = "delete from datos " +
							"where id_concepto=" + c.getId();
		String consulta2 = "";
		
		/*
		 *  Recupero todas las fechas asociadas a los valores que tiene idConcepto
		 *  para despúes borrarlas de la tabla FECHAS
		 */
	...
	private ArrayList<Integer> recuperaFechas(int idConcepto){
		...
		String consulta = "select id_fecha " +
							"from datos " +
		...
		while (cursor.moveToNext()){
			...
			listaFechas.add(Integer.valueOf(cursor.getInt(cursor.getColumnIndex("id_fecha"))));
	 ...
	 */
	public int actualizarValor(double valor, int idConceptoAnterior, int idConceptoNuevo, int fecha){
		
		int registro=1;
		
		String consulta = "update datos " +
						"set valor="+valor+", id_concepto=" + idConceptoNuevo +
						" where id_concepto="+idConceptoAnterior+" and id_fecha="+fecha;
	...
	/*
	 * Método donde se recupera los conceptos de ingresos con sus respectivos valores en euros y
	 * se vuelca a una Lista
	 */
	public ArrayList<ValoresElementoListaGD> obtenerValoresIngresos(int fecha){
		...
		String consulta = "select b.id_concepto,b.id_fecha,a.nombre,b.valor " +
				...
				"and b.id_fecha = "+ fecha +
				" and a.tipo_concepto = 'ingreso'";
		
		Cursor cursor = database.rawQuery(consulta, null);
		
		while (cursor.moveToNext()){
		
			listaVELGD.add(new ValoresElementoListaGD(
					cursor.getInt(cursor.getColumnIndex("id_concepto")),
					cursor.getInt(cursor.getColumnIndex("id_fecha")),				
					cursor.getString(cursor.getColumnIndex("nombre")), 
	...
	/*
	 * Método donde se recupera los conceptos de gastos con sus respectivos valores en euros y
	 * se vuelca a una Lista
	 */
	...
	public ArrayList<Integer> obtenerDiasConValores(int fecha){
		...
		String consulta = "select distinct id_fecha " +
						"from datos " +
						"where id_fecha between " + iFechaInicial.intValue() + " and " + iFechaFinal.intValue() + 
						" order by id_fecha";
		...
		while (cursor.moveToNext()){
			...
			listaODCV.add(Integer.valueOf(cursor.getInt(cursor.getColumnIndex("id_fecha"))));
	...
	/*
	 * Método que me va a Listar las fechas, conceptos y valores asociados entre dos
	 * fechas pasadas por parámetro para los INGRESOS
	 */
	public ArrayList<ValoresElementoListaGD> listadoValoresIngresosPorFecha(int fechaini, int fechafin){
	...
	/*
	 * Método que me va a Listar las fechas, conceptos y valores asociados entre dos
	 * fechas pasadas por parámetro para los GASTOS
	 */
	public ArrayList<ValoresElementoListaGD> listadoValoresGastosPorFecha(int fechaini, int fechafin){
	...
	public void lanzarMensajeError(Exception e){
		Toast t = Toast.makeText(this.context, e.getMessage().toString(), Toast.LENGTH_SHORT);
		t.show();
	}
}
